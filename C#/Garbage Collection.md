 ...
 * 스택에 있는 a는 자연스럽게 메모리에서 사라지지만, 힙에 있는 A는 존재하게 됨
...
* .Net 애플리케이션이 실행되면 JIT 컴파일러가 이 루트들을 목록으로 만들고, CLR은 이 루트 목록을 관ㄷ리하며 상태를 갱신
* 중요한 이유
...
5. 모든 객체 이동이 끝나면, Garbage가 수거된 메모리를 얻을 수 있음
< 3, 4, 5 예시 이미지 >
* 초록색 C, D가 Garbage일 때,
